...
/************************L2 cache **************************/
void accessL2(uint32_t address, uint8_t *data, uint32_t mode, int evict, uint8_t *evicted_dirty_state) {
  ...
  */
  ...
  // we are evicting a block from L1
  if (evict) {
    // if line has dirty block
    if (line->dirty) {
      mem_addr = (line->tag << 15) + (index << 6);
      accessDRAM(mem_addr, &L2Blocks[index * BLOCK_SIZE], MODE_WRITE);
    }
    memcpy(&L2Blocks[index * BLOCK_SIZE], data, BLOCK_SIZE);
    line->valid = 1;
    line->tag = tag;
    line->dirty = *evicted_dirty_state;
  ...
  */
  if ((!line->valid) || line->tag != tag) {
    mem_addr = (tag << 15) + (index << 6);
    accessDRAM(address, tmp_block, MODE_READ); // get new block from DRAM
...
void accessL1(uint32_t address, uint8_t *data, uint32_t mode) {
  ...
  */
  ...
  if (!line->valid || line->tag != tag) {
    ...
    accessL2(mem_addr, tmp_block, MODE_READ, NO_EVICT, NULL); // get new block from DRAM

    // evict this line to L2
    if (line->valid) {
      // memory address of the dirty block in cache
      mem_addr = (line->tag << 14) + (index << 6);
      // write dirty block into memory
      accessL2(mem_addr, &L1Blocks[index * BLOCK_SIZE], MODE_WRITE, EVICT, &line->dirty);
    }
